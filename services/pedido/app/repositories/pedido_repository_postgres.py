...
def _to_domain(model: PedidoModel) -> Pedido:
    ...
    return Pedido(id=model.id, usuario_email=model.usuario_email, itens=itens, status=model.status)
...
class PedidoRepositoryPostgres(PedidoRepository):
    ...
    def save(self, pedido: Pedido) -> Pedido:
        model = PedidoModel(id=pedido.id, usuario_email=pedido.usuario_email, status=pedido.status)
        model.itens = [
            ItemPedidoModel(pedido_id=pedido.id, produto_sku=i.produto_sku,
    ...
    def find_by_usuario(self, usuario_email: str) -> list[Pedido]:
        models = self.session.query(PedidoModel).filter_by(usuario_email=usuario_email).all()
